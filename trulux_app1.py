...
dbmod0.reset_index().to_excel("data_trulux/models.xlsx")
...
dbmod0r=pd.DataFrame(np.array(dbmod0r),columns=list(dbmod0r.columns.astype('string')))
#print(dbmod0r.columns)
for jj in list(dbmod0r.index):
   dbmod0jsn.append(dbmod0r.loc[jj,:].to_dict())
dtsin=dbmod0jsn
crud(dtsname='mongopre_modelsv1',read=False,jsn=True,dtsin=dtsin)
...
dtsname_=f"mongopre_listingsdef{BRND.lower()}v1"
dtsin=dbomelist_expnorpr0jsn
...
ome_catalog.to_excel(f"data_trulux/{BRND.lower()}_cataloguev1.xlsx")
